...
### Review Findings (2026-05-08)

- [ ] [Review][Decision] `pri01_staging_modify_own` policy missing `USING` clause — The FOR ALL policy specifies only `WITH CHECK`, not `USING`. Per PostgreSQL RLS semantics, missing `USING` on a permissive policy defaults to `true`, which means: (a) for SELECT, the SELECT policy `pri01_staging_select_own` correctly restricts visibility (different command types AND together — chained-FK check holds); (b) for UPDATE/DELETE issued via a customer JWT, only the FOR ALL policy applies and `USING true` permits visibility on every row, so a customer could mutate `flushed_at`/`import_id` etc. on any other customer's row (the WITH CHECK only guards the post-update `customer_marketplace_id`). The mitigations in spec are: worker writes via service-role (bypasses RLS), customers are read-only by design, and the RLS regression suite registers this table as `serviceRoleOnly: true`. The implementation matches the spec's exact DDL block (lines 90-107), but the spec is internally inconsistent — it claims to "follow the chained-FK pattern used for sku_channels", and `sku_channels_modify_own`/`baseline_snapshots_modify_own` both have `FOR ALL USING (...) WITH CHECK (...)` (USING + WITH CHECK). Defense-in-depth fix would add `USING ( customer_marketplace_id IN (...) )` matching the spec's WITH CHECK clause. Per the migration-immutability rule, this should not be patched in `202604301214_create_pri01_staging.sql` directly; if accepted, it requires a follow-up migration that runs `ALTER POLICY pri01_staging_modify_own ON pri01_staging USING (...)` (or DROP+CREATE with the same name). Decision needed: (a) accept divergence as spec-authoritative since worker is service-role and customers are read-only, defer hardening to a later RLS-audit story; or (b) add a follow-up ALTER POLICY migration in this story.
- [x] [Review][Defer] Pre-existing test failures in `npm run test:unit` unrelated to Story 5.2 [tests/app/routes/dashboard/dry-run-minimal.test.js, tests/app/routes/onboarding/margin.test.js, tests/shared/mirakl/a01-pc01-of21-p11.test.js] — `dashboard-dry-run-minimal`, `margin-question`, and `ESLint no-direct-fetch` (flagging `app/src/routes/onboarding/scan.js`) all fail on this branch but the underlying source files were never touched by Story 5.1 or Story 5.2 commits. They originate on `main`. deferred, pre-existing.
