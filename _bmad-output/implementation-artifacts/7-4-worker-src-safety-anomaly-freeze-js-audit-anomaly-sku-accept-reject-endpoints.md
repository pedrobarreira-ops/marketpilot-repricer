...
**Pre-authorized scope: `cooperative-absorb.test.js` env-stub patch.** After this AC lands, `cooperative-absorb.js` transitively loads [shared/resend/client.js](shared/resend/client.js) at module-load time via the new static `import` of `anomaly-freeze.js` → resend client. If [tests/worker/engine/cooperative-absorb.test.js](tests/worker/engine/cooperative-absorb.test.js) does NOT already set `RESEND_API_KEY` and use `await import`, the existing Story 7.3 unit tests will fail at module load (throws on missing env var). Pre-authorized: add the `RESEND_API_KEY` env-stub + `await import` pattern at the top of `cooperative-absorb.test.js` per memory `project_resend_env_stub_import_pattern` — same shape as AC8 Task 8 prescribes for `anomaly-freeze.test.js`. Scope: ~5 added lines at file top, no behavioral changes. Do NOT halt for scope question at Task 7 verification — apply the patch and proceed.
...
| [worker/src/engine/decide.js](worker/src/engine/decide.js) | Propagate freeze event slug into `auditEvents` return (verify wiring at Task 4; patch if missing — expected to be required) | AC4 | Likely edit |
| [tests/worker/engine/cooperative-absorb.test.js](tests/worker/engine/cooperative-absorb.test.js) | Add `RESEND_API_KEY` env-stub + `await import` pattern at file top (pre-authorized in AC2 scope guard — module-load throws after AC2 static import without it) | AC2 | Edit |
