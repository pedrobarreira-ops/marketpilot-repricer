...
- **F5 🔴 Critical**: `audit_log_event_types` lookup table referenced as FK target but missing from migrations list. Resolution: added `20260430120730_create_audit_log_event_types.sql` ahead of `_create_audit_log_partitioned.sql`; seeded with AD20 taxonomy (26 base rows); Stories 12.1 + 12.3 each ALTER-INSERT one row → 28 total at end of MVP. **Filename-format note (Story 9.0 dev pass 2026-05-04):** the 14-digit `YYYYMMDDHHMMSS` timestamp is intentional and differs from sibling migrations' 12-digit `YYYYMMDDHHMM` form. The original spec name `202604301207a_create_audit_log_event_types.sql` was rejected by Supabase CLI v2.98.1 at apply-time because the CLI's filename validator requires a fully-numeric prefix (`^[0-9]+_<name>\.sql`). The 14-digit form is purely numeric AND sorts lexicographically between `202604301207_create_baseline_snapshots.sql` (Story 4.2) and `202604301208_create_audit_log_partitioned.sql` (Story 9.1), preserving the F5 ordering invariant — event_types still applies before partitioned, so the partitioned table's FK to event_types resolves at apply-time
