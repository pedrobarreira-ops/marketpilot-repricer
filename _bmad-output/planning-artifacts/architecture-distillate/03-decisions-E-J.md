...
- Sensitive-field redaction list (built into pino config): `Authorization`, `cookie`, `set-cookie`, `password`, `password_hash`, `shop_api_key`, `master_key`, `MASTER_KEY_BASE64`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `RESEND_API_KEY`; any log line containing redacted key replaces value with `'[REDACTED]'`
