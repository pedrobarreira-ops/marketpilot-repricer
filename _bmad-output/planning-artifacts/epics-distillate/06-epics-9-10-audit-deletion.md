...
- **Bob-trace:** SSoT: `worker/src/jobs/monthly-partition-create.js`. Migration: `supabase/migrations/202604301208_create_audit_log_partitioned.sql` (partitioned base table + initial month partition + priority trigger function). Depends on Story 9.0 (event_types lookup must exist for FK), Story 1.1 (worker for cron), Story 2.1 (RLS clients). Enables Story 9.0's `writeAuditEvent` has a table to write to; every event-emitting story across Epics 4-12 produces persisted rows.
- **Acceptance Criteria:**
