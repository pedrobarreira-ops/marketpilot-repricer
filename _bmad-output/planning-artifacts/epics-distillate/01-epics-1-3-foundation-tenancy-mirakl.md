...
- **Bob-trace:** SSoT modules: `app/src/server.js`, `worker/src/index.js`, `worker/src/jobs/heartbeat.js`, `app/src/routes/health.js`, `eslint.config.js`. Migrations: `supabase/migrations/202604301212_create_worker_heartbeats.sql`. First story (no deps).
- **Acceptance Criteria:**
...
- **Bob-trace:** SSoT: `shared/crypto/envelope.js`, `shared/crypto/master-key-loader.js`, `scripts/check-no-secrets.sh`, `scripts/rotate-master-key.md`. Migration: `supabase/migrations/202604301204_create_shop_api_key_vault.sql` (table only — wiring in Epic 4). Depends on Story 1.1. Enables Story 4.3, Story 10.x.
- **Acceptance Criteria:**
  ...
  4. **Given** the secret-scanning hook is installed **When** I attempt `git commit` with a file containing any of `MASTER_KEY`, `shop_api_key`, `sk_live_`, `sk_test_` (the four AD3-locked patterns), OR the heuristic `Authorization: Bearer` **Then** `scripts/check-no-secrets.sh` (configured as a pre-commit hook) blocks the commit and prints which file/pattern matched **And** the hook is idempotent **And** the hook's regex matches `MASTER_KEY_BASE64=...` AND `MASTER_KEY=...` AND any string containing `shop_api_key` (case-sensitive) AND `sk_live_<anything>` AND `sk_test_<anything>` **And** the canonical regexes the hook MUST use (back-ported from Story 1.2 review fixes per Epic 1 retro Item 1) are: `MASTER_KEY[A-Z0-9_]*` (the `0-9` character class is mandatory — without it, `MASTER_KEY_BASE64` only matches up to `MASTER_KEY_BASE` and the secret leaks past the hook), `Authorization:[ \t]*['"]?Bearer` (the optional leading quote is mandatory — without it, source code that writes `Authorization: "Bearer xxx"` with a leading double-quote bypasses the hook), `shop_api_key` (case-sensitive, plain string match), `sk_live_[A-Za-z0-9_]+`, `sk_test_[A-Za-z0-9_]+`.
...
- **Bob-trace:** SSoT: `app/src/routes/_public/signup.js`, `app/src/routes/_public/login.js`, `app/src/routes/_public/forgot-password.js`, `app/src/routes/_public/reset-password.js`, `app/src/middleware/source-context-capture.js`, `app/src/middleware/auth.js`, `app/src/views/pages/signup.eta`, `app/src/views/pages/login.eta`. Migrations: `supabase/migrations/202604301200_create_customers.sql`, `supabase/migrations/202604301201_create_customer_profiles_with_trigger.sql`. Depends on Stories 1.1, 1.3. Enables Story 4.3, every customer-scoped feature.
...
- **Acceptance Criteria:**
  ...
  3. **Given** the signup route receives a request missing `first_name` **When** the trigger raises with `HINT = 'PROFILE_FIRST_NAME_REQUIRED'` **Then** Supabase Auth returns the error to the route **And** the route inspects the error's HINT field and renders a PT-localized field-level error: e.g., *"Por favor introduz o teu nome próprio."* **And** the customer remains on `/signup` with their other fields preserved (email, last_name, company_name) **And** no partial-state `customers` or `auth.users` rows exist (verify with a DB query) **And** when Supabase Auth returns the `user_already_exists` error code (email is already registered to a prior `auth.users` row), the route's `mapSignupError` helper returns `{field: 'email', messagePt: 'Este email já está registado. Tenta iniciar sessão.'}` and the route renders the PT message as an inline field-level error on the email input (NOT a generic page-level error) with the customer's other fields (first_name, last_name, company_name) preserved (Q4 of Epic 1 retro 2026-05-03 — shipped UX is canonical, AC follows shipped truth; this clause back-ported from `app/src/routes/_public/signup.js`'s shipped behavior).
...
- **Bob-trace:** SSoT: `app/src/middleware/founder-admin-only.js`. Migration: `supabase/migrations/202604301202_create_founder_admins.sql`. Depends on Story 1.4 (auth middleware). Enables Story 8.10, every admin-route gate.
- **Acceptance Criteria:**
