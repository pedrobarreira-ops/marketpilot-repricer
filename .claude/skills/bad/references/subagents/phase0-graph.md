...
- **Atomicity-bundle exception (linear stacked-worktree dispatch):** if the story appears in the top-level `merge_blocks:` block in `sprint-status.yaml`, the dispatch uses linear stacking governed by the `bundle_dispatch_orders:` block (sibling of `merge_blocks:`, NOT inside it). Look up the bundle this story belongs to (find which bundle-key list contains the story-key), then identify its **immediate predecessor** in that list (the entry directly above this story). Two conditions must hold:
